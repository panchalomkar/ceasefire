...
class local_trigger_observer {
    ...
    public static function getloginpoints($userid) {
        ...
        if ($record) {
            if ($record->points < $setting->loginpoint_day2) {
                return $setting->loginpoint_day2;
            } else if ($record->points < $setting->loginpoint_day3) {
                return $setting->loginpoint_day3;
            } else if ($record->points < $setting->loginpoint_day4) {
                return $setting->loginpoint_day4;
            } else if ($record->points < $setting->loginpoint_day5) {
                return $setting->loginpoint_day5;
            } else {
                return $setting->loginpoint_day5;
            }
        }
        return $setting->loginpoint_day1;
    }
...
//        print_object($eventdata);die;
        ...
        if ($quiz) {
            ...
            if ($marks >= $gradeitem->gradepass) {
                ...
                if (!$DB->record_exists_sql($SQL)) {
                    ...
                    if (add_point_log($userid, 'quiz', 'added', $setting->dailyquiz_point)) {
    ...
    public static function newusercreated1(\core\event\user_created $event) {
        ...
        add_point_log($userid, 'welcome', 'added', $setting->welcome_point);
    }
        ...
        if ($course) {
            ...
            if ($marks >= $gradeitem->gradepass) {
                ...
                if (!$DB->record_exists('user_scratchcard', array('userid' => $userid, 'itemid' => $quiz->id, 'card_type' => 'quiz'))) {
                    ...
                    $count = $config->quiz_scratch_card;
                    for ($i = 0; $i < $count; $i++) {
